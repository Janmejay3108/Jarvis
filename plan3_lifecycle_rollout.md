...
**Reuse the Plan1 mechanism — do not build a second one** (`docs/agent/decisions/004-jira-action-reconciliation.md`). Every lifecycle call is a `jira_actions` row with the same states, the same `jira.action.updated` event, and the same action-scoped check/retry routes built in plan1 §1.1.2 / §1.5.3 / §1.6.2. A separate retry path here is a **defect**.
- Writes stay **single-attempt**; a timeout/5xx is `uncertain`, not failed (plan1 §1.2.1, D003). **Check before any retry** — attachments and transitions are non-idempotent, so a blind retry duplicates evidence or advances workflow twice.
- **Reconciliation reads per operation:** comment → match the `action_id` footer; label → the issue's exact label set; **attachment → filename + size + creation-time on the issue's attachment array** (store an artifact reference and safe metadata, **never the bytes**); **transition → the issue's current status plus its changelog**, since an unavailable transition (a legitimate no-op) is not the same as an attempted transition whose outcome is unknown.
**DoD:** on a test ticket: comment + attachments + label appear; blocked transition skipped gracefully; **an uncertain attachment or transition is recoverable through the plan1 check/retry surface without duplicating it, and no lifecycle failure fails the run.**
