...
- `trigger(test_config_id)` — `POST /task_scheduler_service/api/v1/task_instances/{test_config_id}` → **201** with a `task_instance_id` (canonical in plan_master §2.3.5; **proven live 2026-08-02**). One transient **500** was observed and the identical retry returned 201 — do not treat a single 500 as a contract error.
- The **five-endpoint results/evidence chain** (plan_master §2.3.4): `GET /api/v2/test_config_results?test_config_id=<ID>` → newest result id; `GET /api/v2/test_results?test_config_result_id=<id>` → the `run_id` + step status; `GET /api/v2/test_results/{run_id}/logs?limit=1000` → entries (`message`, `severity`, `message_type`, `image_id`); `GET /api/v2/test_results/{run_id}/screenshots` → the run's screenshot list; `GET /api/v2/screenshots/{screenshot_id}` → PNG (PoC-2 walk-back logic reused). **One identifier, one name** — `{run_id}` in the last three routes is the test-result id from the second call, not a separate lookup. **The screenshot list call is required** — a PNG cannot be fetched straight from a log entry's `image_id` on this DAI. **`GET /testconfiguration/{id}/results` 404s on this DAI** — it is not the results route; use `test_config_results` above.
...
6. on completion, walk the **five-endpoint results/evidence chain** for status + logs + screenshots + `result_url`;
