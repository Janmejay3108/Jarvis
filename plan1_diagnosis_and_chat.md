...
- `complete(system_blocks, messages, model=None, max_tokens=4096, tools=None, tool_choice=None, thinking=False, images=None, output_model=None) -> (response, usage)` — `model=None` resolves to the single Opus ID; the parameter exists only for the optional light-utility calls. **Return type depends on the call:** a forced structured call (`tool_choice={"type":"tool",…}` + `output_model`) returns `(validated_dict, usage)`; every other call returns `(full_message, usage)`.
...
- **Structured outputs [UP-2]:** when `tools` + `tool_choice={"type":"tool","name":...}` given, return the tool-use input dict directly; on pydantic validation failure, one auto-repair retry feeding the validation errors back. **Two distinct schemas, never conflated** (Decision 005): `tools[].input_schema` is the **JSON schema sent to Anthropic** and describes the request only; **`output_model: type[BaseModel]` is the local pydantic validator** that runs `model_validate(...)` on the returned tool input and then `model_dump(mode="json")`. A JSON schema alone cannot execute pydantic field/model validators, so it cannot satisfy the repair-on-validation-failure rule above. **`output_model` is required if and only if `tool_choice.type == "tool"`**; every incoherent combination (forced tool without `output_model`, `output_model` without a forced tool, unknown or duplicated forced name) fails **preflight, before any HTTP request**. **Exactly one** semantic repair is permitted: append the original assistant content unchanged (including `thinking`/`redacted_thinking` blocks), then one `tool_result` with `is_error: true` carrying **field paths and error types only** — never `repr(input)`, the invalid payload, prompts, credentials or image bytes. If the second response is still invalid, raise `StructuredOutputError`. **This repair is not a transport retry and is not plan2's fix loop** — see the boundary note below.
- **Cost meter [UP-13]:** maintain a per-model price table in `config/enovia.yaml` (`llm.prices`, Opus rates); compute `cost_usd` per call incl. cache-read/-write token classes; expose `usage` to callers. **Every physical response is charged immediately, at the moment it is received** — including the first response of a repaired pair, and including a response that itself trips the cap. A repaired structured call therefore charges **both** responses and reports `request_count == 2`; transport attempts that never returned a response are never charged. `utils/budget.py BudgetGuard(limit)` — `charge(cost)` raises `BudgetExceeded` past the cap (default **$10/run**); pipeline converts to a graceful `run.failed` with a clear message. **A first-response charge that exceeds the budget prevents the repair** rather than spending more.
- **Thinking [UP-15]:** `thinking=True` sets the extended-thinking param on the same Opus model (used on retry attempts in plan2). **Verified 2026-08-04:** `claude-opus-4-7` **rejects** `thinking: {"type":"enabled", budget_tokens:…}` with HTTP 400 — send `thinking: {"type":"adaptive"}` with `output_config: {"effort":"high"}`. Adaptive thinking **is** compatible with forced tool choice, so structured output and thinking can be combined.
- `tenacity` retry on 429/5xx/timeouts with jitter; structlog every call (model, latency, tokens, cache hits, cost). **The SDK's own retries must be disabled (`max_retries=0`) so they do not multiply tenacity's** — three total transport attempts, not nine. Log **allowlisted safe fields only**: never prompts, messages, tool payloads, image data, keys, URLs, headers or exception text.

**Three separate boundaries — never collapse them into one counter:**

| Boundary | Count | Scope |
|---|---|---|
